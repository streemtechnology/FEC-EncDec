...
def receive_write_stream():
    ...
    reconstruct_rtmp = librtmp.RTMP("rtmp://172.17.66.85:1935/show/stream_name", live=True)
...
def decode_from_fec(frame,row,data,stream):
    ...
    #print frame, cur_send, frame != cur_send
    if frame != cur_send:
        print "OUT OF ORDER FRAME DETECTED Processing: ", cur_send, "GOT FRAME:", frame 
        if frame == 0 :
            for i in range(cur_send, 8):
                print "FLUSHING FRAMES:", i
                sequence[i] = range(16)
                in_progress[i] = 0
                sequence[i][row] = 0
        elif frame > cur_send:
            for i in range(cur_send, frame):
                print "FLUSHING FRAMES:", i
                sequence[i] = range(16)
                in_progress[i] = 0
                sequence[i][row] = 0
    ...
    if in_progress[frame]<16:
        ...
        in_progress[frame]+=1
        #print "added to frame ", frame, " block" , row, "in_progress", frame, in_progress[frame]

    # if received all blocks
    for index in xrange(cur_send,8):
        #print "index" , index, "in_progress[index]", in_progress[index]
        if in_progress[index]==16:
            try:
                ...
                in_progress[index] = 0
                cur_send = (index+1)%8
                print "cur_send", cur_send, "sequence", sequence[index]
            except IOError as e:
                print "I/O Error" , e
        else:
            return
...
def construct_rtmp(blocks,stream, frame):
    ...
    for block in blocks:
        data+=block
    stream.write(data)
    print "Wrote frame to rtmp" , frame
